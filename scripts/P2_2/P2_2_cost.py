...
class Classification_cost(nn.Module): 
...
def validate(model, val_loader):
    ...
    with torch.no_grad():
        ...
        for X, y in val_loader:
            ...
            loss = custom_loss(outputs, y)  # Calculate loss
...
def custom_loss(outputs, targets):
    # Obtain the predicted indices with the highest probability
    pred_indices = torch.argmax(outputs, dim=1)
    
    # Extract the predicted costs corresponding to the predicted indices
    pred_cost = torch.gather(targets, 1, pred_indices.unsqueeze(1))
    
    # Obtain the true indices with the lowest cost values
    true_indices = torch.argmin(targets, dim=1)
    
    # Extract the true values corresponding to the true indices
    true_values = torch.gather(targets, 1, true_indices.unsqueeze(1))
    
    # Calculate the regret by subtracting the predicted cost from the true values
    regret = pred_cost - true_values
    
    # Define the loss function as CrossEntropyLoss with no reduction
    loss_fn = nn.CrossEntropyLoss(reduction='none')
    
    # Calculate the loss by multiplying the loss from the loss function with the regret (element-wise)
    loss = loss_fn(outputs, true_indices) * (regret.reshape(-1))
    
    # Return the mean of the calculated loss
    return loss.mean()

        


def classification_cost_model(data,path):
    ...
    train_std[train_std == 0] = 1e-8    # avoid dividing by zero
    ...
    num_epochs = 246  # Number of times the entire training dataset is passed through the model during training
    batch_size = 250 # Number of samples per batch used for training and evaluation
    learning_rate = 0.010335842378671646  # The rate at which the model learns from the training data
    weight_decay = 7.629790649757089e-06  # A regularization technique that helps prevent overfitting by adding a penalty to the loss function
    ...
    # Create the model
    model = Classification_cost(input_size, output_size, hidden_layer_size)
    ...
    patience = 60 #patience is high for this model
    ...
    for epoch in range(num_epochs):
        ...
        val_loss = validate(model, val_loader)  # Add validation loss computation during training
    ...
    loss_plot(train_losses, 'Classification_P1_2')  # Generate the training loss plot
    # Save the model
    model_details = {
