...
from P2_2_cost import ClassificationDataset, Classification_cost, custom_loss
import os
...
def main():
    ...
    model = Classification_cost(input_size, output_size, hidden_size)
   ...
   #using the loss function used in training
    ...
    #print(f"\nFinal results: loss: {avg_loss:8.4f}, \taccuracy: {accuracy:4.4f}, \tavg_cost: {avg_cost:8.4f}, \tsbs_cost: {sbs_avg_cost:8.4f}, \tvbs_cost: {vbs_avg_cost:8.4f}, \tsbs_vbs_gap: {sbs_vbs_gap:2.4f}", file=open(f"../results/classification_advanced_result.txt","a"))



    result_file_path = "results/classification_advanced_result.txt"
