...
def regression_model(data, path):
#def regression_model():
    ...
    num_epochs = 133  # Number of times the entire training dataset is passed through the model during training
    batch_size = 31  # Number of samples per batch used for training and evaluation
    learning_rate = 0.039312787894877646  # The rate at which the model learns from the training data
    weight_decay = 1.954385454487943e-06  # A regularization technique that helps prevent overfitting by adding a penalty to the loss function

    # Create the data loaders
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)  # DataLoader for the training dataset
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)  # DataLoader for the validation dataset

    # Create the regression model
    model = Regression(input_size, output_size, hidden_layer_size)  # Create an instance of the regression model

    # Define the loss function
    criterion = nn.MSELoss()  # Mean Squared Error (MSE) loss function for regression

    # Define the optimizer
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)  # Adam optimizer with specified learning rate and weight decay
    ...
    loss_plot(train_losses, 'regression_P1')  # Generate the training loss plot
    # Save the model
    model_details = {
    ...
    #path = '../models/regression_model.pth'
